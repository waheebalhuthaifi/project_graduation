...
// console.log(eventOrder.order.seller_id);


const storeNotifications = (data) => {
  console.log("data.eventOrder.message",data)
  axios
    .post("/create/notifications",{
      message:data.message,
      user_id:data.order.seller_id,
      is_read: 0,
      service_id:data.order.service_id,
...
const fetchNotifications = async () => {
  try {
    const { data } = await axios.get(`/notification/${user?.id}`);
    // Check if the data is an array
    if (Array.isArray(data)) {
      setNotifications(data);
      console.log("fetchNotifications", data);
    } else {
      // If the data is not an array, you may need to extract the notifications array from the response
          ...
          {user ? (
            <>
              ...
              <ul className="flex w-48 justify-between sm:w-1/4 items-center gap-2 md:mt-2 md:gap-5">
                ...
                <li>
                  <FaBell className="inline mt-1 text-gray-700 rounded-full w-10 h-10 md:w-8 md:h-8  " />
                </li>
                <li>
